...
def severDB(DB_dump):
    ...
    location -- current directory, which should contain the DB data dump"""
	#Opens the full DB dump file and reads it into a list, raw_data.
	input_file = open(DB_dump, 'r')
...
def organism_pull(goodECInfo):
	...
	ecOrgs = {' '.join(x.split('\t')[1].split(' ')[1:]).strip('<>0123456789, \n'): x.split('\t')[1].split(' ')[0] for x in goodECInfo if x[0:3] == 'PR\t'}
		...
		for rxn in rxnInfos:
			...
			for t in left:
				if re.search(r'^[0-9]* ', t) is not None:
					leftside.append(re.split(r'^[0-9]* ', t)[1])
				else:
					leftside.append(t)
			for t in right:
				if re.search(r'^[0-9]* ', t) is not None:
					rightside.append(re.split(r'^[0-9]* ', t)[1])
				else:
					rightside.append(t)

			rxnDict[j] = (oList, leftside, rightside)
...
def TC_analysis(EC_dict, target_substrate, filenamechoice, smilesFile, inchiFile):
	...
	stats_file.write('DB ENZYME SEARCH RESULTS\nRESULTS GIVEN AS:\nTC\tNATURAL SUBSTRATE\tEC NUMBER\n')
...
def SimZyme(DB_dumpfile, substrate, smilesDump, inchiDump, outName, ecNumbers, ecFlag, severFlag):

	dumpLocation = os.getcwd() + '/' + DB_dumpfile

	if severFlag:
		severDB(dumpLocation)

	if ecFlag:
		ecInformation = getECInfo(ecNumbers)
		substrateEcDict, productEcDict = substrate_pull(ecInformation)
		ll = TC_analysis(substrateEcDict, substrate, outName, smilesDump, inchiDump)
	else:
